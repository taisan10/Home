...
//   return (
//     <section className="relative overflow-hidden bg-black text-white py-20 -mt-26 sm:-mt-35 md:-mt-32 lg:-mt-40">
...
export function ChallengesSection() {
  ...
  useEffect(() => {
    ...
    else if (width < 1024) setScreenSize("sm-md");
    else setScreenSize("lg");
  }, []);

  useEffect(() => {
    if (screenSize === "mobile") {
      setVisibleItems(helpItems.slice(0, 3));
      const timer1 = setTimeout(() => setVisibleItems(helpItems.slice(3, 6)), 3000);
      const timer2 = setTimeout(() => setVisibleItems(helpItems.slice(6, 9)), 6000);
      return () => {
        clearTimeout(timer1);
        clearTimeout(timer2);
      };
    ...
    if (screenSize === "sm-md") {
      setVisibleItems(helpItems.slice(0, 4));
      const timer = setTimeout(() => setVisibleItems(helpItems.slice(5, 9)), 3000);
      return () => clearTimeout(timer);
    }

    if (screenSize === "lg") {
      setVisibleItems(helpItems);
    }
  }, [screenSize]);

  return (
    <section className="relative overflow-hidden bg-neutral text-white py-20 -mt-26 sm:-mt-35 md:-mt-32 lg:-mt-40">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_60%_at_50%_10%,rgba(14,165,233,.15),transparent),radial-gradient(40%_40%_at_10%_80%,rgba(20,184,166,.12),transparent)]" />
      <Container>
        <SectionTitle
          title={
        ...
        >
          {visibleItems.map((item, idx) => (
            <div
              key={idx}
              className="group relative bg-neutral-900 border border-transparent rounded-2xl p-6 flex flex-col justify-start transition duration-300 hover:border-purple-500/70 hover:shadow-[0_0_0.5rem_0.2rem_rgba(168,85,247,0.2)] h-full"
            >
              <div className="text-4xl text-white/30 mb-4">
                {/* SVG icon */}
              </div>
